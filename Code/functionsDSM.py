...
def dot_product_tf(x, kernel, axis1=1, axis2=1):
    ...
    if tuple_size == 2:
        ...
        print(dot_product)
    if tuple_size == 3:
        #dot_product = dot([x, kernel], axes=1)
        #dot_product = dot([x, K.transpose(kernel)], axes=1)
        #dot_product = Dot(axes=2)([x, kernel])
        #dot_product = Dot(axes=(1,1))([x, K.transpose(kernel)]) #6*6
        #dot_product = Dot(axes=(1,2))([x, K.transpose(kernel)])
        #dot_product = Dot(axes=(1,1))([x, kernel]) #6*6
        
        dot_product = Dot(axes=(axis1,axis2))([x, kernel]) #5*5
        print(dot_product)
...
def dot_product(x, kernel):
    ...
    if K.backend() == 'tensorflow':
        ...
        print(expand_dim)
        dot_product = K.dot(x, expand_dim)
        print(dot_product)
        result = K.squeeze(dot_product, axis=-1) #remove last dim
        print(result)
        return result
    else:
...
def softmax(x, axis=-1):
    ...
    if ndim == 2: 
        ...
        e = K.exp(x)
        print(e)
        s = K.cast(K.sum(e, axis=axis, keepdims=True), K.floatx())
        print(s)
        result = e / s
        print(result)
        return result
