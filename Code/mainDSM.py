...
def string_embedding(dataset):
    ...
    aux1 = fill_table_input(XA1, chars, char_labels, max_seq_len, method, file_name)
    print(XA1)
    #print(aux1)
    ...
    aux1 = fill_table_input(XB1, chars, char_labels, max_seq_len, method, file_name)
    XB1 = aux1
    del aux1
    
    file_name = 'newfile3.dat'
    aux1 = fill_table_input(XA2, chars, char_labels, max_seq_len, method, file_name)
    XA2 = aux1
    del aux1
    
    file_name = 'newfile4.dat'
    aux1 = fill_table_input(XB2, chars, char_labels, max_seq_len, method, file_name)
    XB2 = aux1
    del aux1
...
def evaluate_string_dnn(dataset='dataset-string-similarity.txt', max_seq_len = 40, training_percent=0.7, num_epochs = 2, 
                        method='gru', bidir_num_layers=2, bidirectional=True, bidir_hiddenunits=60, 
                        hnn_maxpooling=False, hnn_num_layers=3,
    ...
    aux1, time1 = deep_neural_net_rnn(train_data_1=XA1[0],
                                      ...
                                      bidir_num_layers = bidir_num_layers, bidirectional=bidirectional, method = method, bidir_hidden_units=bidir_hiddenunits, 
                                      hnn_maxpooling=hnn_maxpooling, hnn_num_layers=hnn_num_layers,
    ...
    display_metrics(aux1, aux2, Y1, Y2, learn_Y1, timer, method, bidirectional, 
                    shortcut, maxpooling, selfattention, alignment, onlyconcat, training_instances)
    sys.stdout.flush()
...
#Run model
max_seq_len = 250 # max length of sequence, space matter
training_percent = 0.7
num_epochs = 10

# Encoder
method = 'lstm' # 'lstm' #'gru' 'SimpleRNN'
bidir_num_layers = 5
bidirectional = True 
bidir_hiddenunits = 40 #60 
...
maxpooling = False #False
selfattention = False
attention_method = 'proba' #'softmax' 

# Decoder
dnn_num_layers = 3
dnn_hiddenunits = 40 #60

  
evaluate_string_dnn(dataset=vp, max_seq_len = max_seq_len, training_percent=training_percent, num_epochs = num_epochs,
                         method = method, bidir_num_layers = bidir_num_layers, bidirectional = bidirectional, bidir_hiddenunits = bidir_hiddenunits, 
                         hnn_maxpooling = hnn_maxpooling, hnn_num_layers = hnn_num_layers,
